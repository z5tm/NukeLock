...
public sealed class Config : IConfig
{
    ...
    public string RadiationDeathReason { get; set; } = "Died to radiation post-nuclear blast.";
    ...
    public string RadiationWarningMessage { get; set; } =
        "<color=red>The Alpha Warhead has been detonated, Radiation will occur in 2 Minutes</color>";
    ...
    public int AutoNuke { get; set; } = 1200;

    [Description(
        "After what autonuke time should a permanently broadcast be broadcasted across the map to tell the AutoNuke will be activated? $(COUNTDOWN) will be replaced with the time left until autonuke will be activated")] // me?? changing it to $() for no reason?? hell yeah!
    public int AutoNukePermaBroadcastTimer { get; set; } = 30;

    public string AutoNukePermaBroadcastMessage { get; set; } =
        "<color=red>Alpha Warhead Emergency Detonation Sequence will engage in $(COUNTDOWN) seconds</color>";
    ...
    public ushort BeginAutoNukeDetonationBroadcastTime { get; set; } = 3;

    [Description("What broadcast should be broadcasted when the AutoNuke activates?")]
    public string BeginAutoNukeDetonationBroadcastMessage { get; set; } =
        "<color=red>The Alpha Warhead Detonation Sequence engaged, <b>This Warhead cannot be stopped</b></color>";

    [Description(
        "Whether a broadcast will be shown to all players to tell how many seconds are remaining until the warhead explodes.")]
    ...
    public string WarheadDetonationMessage { get; set; } =
        "<color=red>Alpha Warhead detonates in T-Minus</color> <color=orange>%COUNTDOWN% seconds</color>";

    [Description("Whether team respawning will be disabled after warhead detonation.")]
    ...
    public Colors WarheadColor { get; set; } = new();

    [Description("After what time should a cassie message be broadcasted? (Empty = Disabled)")]
    public SortedDictionary<int, string> CassieWarnings { get; set; } = new()
    {
        {
            600,
            "Attention, all personnel, The Alpha Warhead Emergency Detonation Sequence will be started in TMinus 10 Minutes"
        },
    ...
    [Description("Whether to sync the CASSIE announcements so the end of the message will sync with the time provided.")]
    public bool CassieWarningsMessageCalculations { get; set; } = true;
}